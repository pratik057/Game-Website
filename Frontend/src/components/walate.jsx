...
const WalletControls = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gray-900 p-4 sm:p-6">
      <div className="w-full max-w-5xl bg-gradient-to-br from-gray-800 to-gray-700 p-6 sm:p-8 rounded-2xl shadow-2xl text-white border border-gray-700 flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/3 flex flex-col items-center text-center bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg border border-gray-700">
          ...
          <div className="flex w-full bg-gray-700/50 rounded-lg p-1">
            <button className={`flex-1 py-3 rounded-md flex items-center justify-center gap-2 transition-all duration-300 ${activeTab === "deposit" ? "bg-green-600 text-white shadow-md" : "text-gray-300 hover:bg-gray-700"}`} onClick={() => setActiveTab("deposit")}> <ArrowDownCircle size={20} /><span>Deposit</span></button>
            <button className={`flex-1 py-3 rounded-md flex items-center justify-center gap-2 transition-all duration-300 ${activeTab === "withdraw" ? "bg-red-600 text-white shadow-md" : "text-gray-300 hover:bg-gray-700"}`} onClick={() => setActiveTab("withdraw")}> <ArrowUpCircle size={20} /><span>Withdraw</span></button>
          </div>
          ...
          <motion.button className={`w-full mt-4 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition-all ${activeTab === "deposit" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}`} onClick={handleDeposit} disabled={loading}> {loading ? <Loader2 className="animate-spin mr-2" size={20} /> : <ArrowDownCircle size={20} />} <span>{loading ? "Processing..." : activeTab === "deposit" ? "Add Funds" : "Withdraw Funds"}</span></motion.button>
        </div>
