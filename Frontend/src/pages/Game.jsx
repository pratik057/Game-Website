      ...
      <div className="min-h-screen w-full flex flex-col justify-center items-center bg-gradient-to-b from-gray-900 to-black text-white px-4 md:px-8 lg:px-16">
        ...
        <div className="w-full max-w-5xl mt-6 p-4 bg-gray-800 rounded-xl shadow-lg flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center">
            ...
            <span className={`font-bold ${gameState.status === "betting" ? "text-yellow-500" : gameState.status === "dealing" ? "text-blue-500" : gameState.status === "result" ? "text-green-500" : "text-gray-400"}`}>
              {gameState.status === "betting" ? "Betting Open" : gameState.status === "dealing" ? "Dealing Cards" : gameState.status === "result" ? "Round Complete" : "Waiting"}
            </span>
          </div>
        ...
        <div className="w-full flex justify-center mt-6">
          <BetControls onPlaceBet={placeBet} disabled={gameState.status !== "betting" || currentBet !== null} currentBet={currentBet} />
        </div>
